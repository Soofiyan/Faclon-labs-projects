...
#define RIL_HTTP_DEBUG_ENABLE 1
...
//    }
    //http_url_addr = strURL;
    //http_url_addr_len = len;
    m_httpAction = HTTP_ACTION_CONFIG;
    Ql_sprintf(strAT, "AT+QHTTPCFG=\"content-Type\",\"application/json\"\0");
    retRes = Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), ATRsp_QHTTP_Handler, &errCode, 0);
    if (retRes != RIL_AT_SUCCESS)
    {
        if (RIL_AT_FAILED == errCode)
...
//            Ql_Sleep(3000);
//            Ql_RIL_RcvDataFrmCore(http_post_msg,1024,1);
//            RIL_HTTP_DEBUG(DBG_Buffer, "%s\r\n",http_post_msg);
        }
